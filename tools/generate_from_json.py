...
OVERVIEW:
This utility generates audiobook chunks directly from pre-processed JSON files,
bypassing text analysis and chunking steps. It's essential for debugging TTS
parameters, testing voice cloning, and regenerating specific chunks with
custom settings.

PRIMARY PURPOSES:
1. DEBUGGING: Test TTS parameters without full text processing
2. PARAMETER TUNING: Experiment with temperature, CFG, exaggeration settings
3. SELECTIVE REGENERATION: Regenerate specific chunks after editing
4. VOICE TESTING: Test different voice samples on same text chunks
5. QUALITY IMPROVEMENT: Re-process chunks with refined settings

JSON WORKFLOW BENEFITS:
- SPEED: Skips text chunking and sentiment analysis steps
- PRECISION: Direct control over TTS parameters per chunk
- FLEXIBILITY: Easy editing of chunk text and settings in JSON
- REPRODUCIBILITY: Exact parameter settings preserved in JSON
- EFFICIENCY: Memory-optimized processing with caching

TECHNICAL FEATURES:
- In-memory processing pipeline (no temp files)
- Voice embedding pre-warming and caching
- Parallel chunk processing with ThreadPoolExecutor
- Real-time progress tracking and ETA calculation
- Memory-safe processing with VRAM monitoring
- Resume capability for interrupted processing

INPUT REQUIREMENTS:
- Pre-existing chunks_info.json file (from text processing step)
- Voice sample files in Voice_Samples/ directory
- Proper ChatterboxTTS model installation

OUTPUT:
- Individual chunk_XXXXX.wav files in audio_chunks/ directory
- Processing logs and performance metrics
- Ready for audio combination into final audiobook

USAGE SCENARIOS:
- Testing voice cloning with different samples
- Fine-tuning emotional expression parameters
- Regenerating chapters with updated settings
- Quality control and iterative improvement
- Debugging problematic chunks in isolation
"""
