...
# Audio controls moved to main button area next to Regenerate M4B
        ...
        self.avg_its_label = QLabel("-- it/s")
...
# Audio controls row removed from status panel
        ...
        layout.addRow("Avg it/s:", self.avg_its_label)
    ...
    def update_status(self, operation=None, progress=None, elapsed=None, eta=None,
        ...
        if progress is not None:
            if isinstance(progress, tuple):  # (current, total)
                ...
                # Also reflect chunk progress in the label if no explicit chunk_info provided
                if not chunk_info:
                    self.current_chunk_label.setText(f"Chunk {current}/{total}")
            else:
                self.progress_bar.setVisible(False)
        if elapsed:
        ...
        if avg_its:
            # Accept either "0.3 it/s" or just a float string
            if not str(avg_its).endswith("it/s"):
                self.avg_its_label.setText(f"{avg_its} it/s")
            else:
                self.avg_its_label.setText(avg_its)
        if vram:
...
class ProcessThread(QThread):
    ...
    def parse_and_emit_status(self, status_text):
        ...
        # Extract average it/s
        avg_match = re.search(r'Avg:\s*([0-9.]+) it/s', status_text)
        if avg_match:
            status_data['avg_its'] = avg_match.group(1)
...
class ChatterboxMainWindow(QMainWindow):
    def __init__(self):
        ...
        # Initialize token usage logging for TTS performance analysis
        self.init_token_logging()

        # Test audio system on startup (Windows compatibility)
        self.test_audio_system_startup()

    def init_token_logging(self):
        """Initialize simple token usage logging"""
        try:
            from modules.simple_token_logger import init_token_log
            init_token_log()
            self.log_output("📊 Simple token usage logging enabled")

        except Exception as e:
            self.log_output(f"⚠️ Failed to initialize token logging: {e}")
...
#        vader_layout.addWidget(vader_info)
        ...
        # Smoothing window
        self.smoothing_window_spin = NoScrollSpinBox()
        self._attach_spin_reset(self.smoothing_window_spin, 'SENTIMENT_SMOOTHING_WINDOW')
...
#        main_quality_layout.addWidget(quality_info)
        ...
        self._attach_spin_reset(self.seed_spin, 'DEFAULT_SEED')
        ...
        # Min-P
        self.min_p_spin = NoScrollDoubleSpinBox()
        self._attach_spin_reset(self.min_p_spin, 'DEFAULT_MIN_P')
        ...
        # Top-P
        self.top_p_spin = NoScrollDoubleSpinBox()
        self._attach_spin_reset(self.top_p_spin, 'DEFAULT_TOP_P')
...
# Audio button connections moved to main button area creation
    ...
    def handle_micro_batching_toggle(self, state):
        from config import config  # Import config module directly
        is_checked = bool(state)
        config.ENABLE_MICRO_BATCHING = is_checked
        config.ENABLE_VADER_MICRO_BATCHING = is_checked
        try:
            import logging
            logging.info(f"Micro-batching globally set to: {is_checked}")
        except Exception:
            pass

    def handle_vader_toggle(self, state):
        """When VADER is turned off, also turn off micro-batching (Tab 2) and config flags."""
        is_checked = bool(state)
        try:
        ...
        if not is_checked:
            # Uncheck the micro-batching checkbox in Tab 2 if present
            try:
                if hasattr(self, 'micro_batching_checkbox') and self.micro_batching_checkbox.isChecked():
                    self.micro_batching_checkbox.setChecked(False)
            except Exception:
                pass
            # Ensure config flags are off even if Tab 2 not initialized yet
            try:
                if _cfg is not None:
                    _cfg.ENABLE_MICRO_BATCHING = False
    ...
    def _build_effective_settings(self):
        ...
        normalize = {
            ...
            'VADER_CFG_WEIGHT_SENSITIVITY': 'vader_cfg_sensitivity',
    ...
    def reload_tab1_from_config(self):
        try:
            ...
            mapping = [
                ...
                (self.seed_spin, 'DEFAULT_SEED'),
                (self.exaggeration_spin, 'DEFAULT_EXAGGERATION'),
                (self.cfg_weight_spin, 'DEFAULT_CFG_WEIGHT'),
    ...
    def create_config_tab(self):
        ...
        self.workers_spin = NoScrollSpinBox()
        self._attach_spin_reset(self.workers_spin, 'MAX_WORKERS')
        ...
        self.tts_batch_size_spin = NoScrollSpinBox()
        self._attach_spin_reset(self.tts_batch_size_spin, 'TTS_BATCH_SIZE')
        ...
        self.batch_chunk_size_spin = NoScrollSpinBox()
        self._attach_spin_reset(self.batch_chunk_size_spin, 'BATCH_CHUNK_SIZE')
        self.batch_chunk_size_spin.setRange(1, 16)
        self.batch_chunk_size_spin.setValue(BATCH_CHUNK_SIZE if 'BATCH_CHUNK_SIZE' in globals() else 6)
        self.batch_chunk_size_spin.setMaximumWidth(60)
        workers_layout.addRow("Microbatch Chunk Size:", self.batch_chunk_size_spin)

        # Max New Tokens setting
        self.max_new_tokens_spin = NoScrollSpinBox()
        self._attach_spin_reset(self.max_new_tokens_spin, 'MAX_NEW_TOKENS')
        self.max_new_tokens_spin.setRange(100, 2000)
        self.max_new_tokens_spin.setValue(getattr(config_mod, 'MAX_NEW_TOKENS', 425))
        self.max_new_tokens_spin.setMaximumWidth(80)
        workers_layout.addRow("Max New Tokens:", self.max_new_tokens_spin)
        # Micro-batching toggle
        self.micro_batching_checkbox = QCheckBox("Enable micro-batching (global)")
        try:
            from config.config import ENABLE_MICRO_BATCHING as _EMB
        except Exception:
            _EMB = False
        self.micro_batching_checkbox.setChecked(bool(_EMB))
        self._attach_config_key(self.micro_batching_checkbox, 'ENABLE_MICRO_BATCHING')
        self.micro_batching_checkbox.stateChanged.connect(self.handle_micro_batching_toggle)
        workers_layout.addRow("Enable Micro-batching:", self.micro_batching_checkbox)
        ...
        self.exag_min_spin = NoScrollDoubleSpinBox()
        self._attach_spin_reset(self.exag_min_spin, 'TTS_PARAM_MIN_EXAGGERATION')
        ...
        self.exag_max_spin = NoScrollDoubleSpinBox()
        self._attach_spin_reset(self.exag_max_spin, 'TTS_PARAM_MAX_EXAGGERATION')
        ...
        self._attach_spin_reset(self.vader_cfg_sens_spin, 'VADER_CFG_WEIGHT_SENSITIVITY')
        ...
        # Inline pause settings (~1, ~2)
        inline_group = QGroupBox()
        inline_group.setStyleSheet("QGroupBox { border: 1px solid gray; border-radius: 3px; margin: 5px; padding-top: 10px; }")
        inline_layout = QHBoxLayout(inline_group)
        try:
            from config.config import ENABLE_INLINE_PAUSES, INLINE_PAUSE_1_MS, INLINE_PAUSE_2_MS
        except Exception:
            ENABLE_INLINE_PAUSES, INLINE_PAUSE_1_MS, INLINE_PAUSE_2_MS = True, 150, 300
        self.inline_toggle = QCheckBox("Enable Inline Pauses (~1, ~2)")
        self.inline_toggle.setChecked(bool(ENABLE_INLINE_PAUSES))
        self._attach_config_key(self.inline_toggle, 'ENABLE_INLINE_PAUSES')

        self.inline_1_spin = NoScrollSpinBox()
        self._attach_spin_reset(self.inline_1_spin, 'INLINE_PAUSE_1_MS')
        self.inline_1_spin.setRange(0, 5000)
        self.inline_1_spin.setValue(INLINE_PAUSE_1_MS)
        self.inline_1_spin.setMaximumWidth(60)

        self.inline_2_spin = NoScrollSpinBox()
        self._attach_spin_reset(self.inline_2_spin, 'INLINE_PAUSE_2_MS')
        self.inline_2_spin.setRange(0, 5000)
        self.inline_2_spin.setValue(INLINE_PAUSE_2_MS)
        self.inline_2_spin.setMaximumWidth(60)

        inline_layout.addWidget(self.inline_toggle)
        inline_layout.addSpacing(10)
        inline_layout.addWidget(QLabel("~1 (ms):"))
        inline_layout.addWidget(self.inline_1_spin)
        inline_layout.addSpacing(10)
        inline_layout.addWidget(QLabel("~2 (ms):"))
        inline_layout.addWidget(self.inline_2_spin)
    ...
    def create_combine_tab(self):
        ...
        # Combine buttons
        combine_buttons_layout = QHBoxLayout()

        # Combine to WAV only
        combine_wav_btn = QPushButton("🔗 Combine to WAV")
        combine_wav_btn.setStyleSheet("QPushButton { background-color: #9C27B0; color: white; font-weight: bold; padding: 10px; }")
        combine_wav_btn.clicked.connect(self.combine_audio_wav_only)
        combine_buttons_layout.addWidget(combine_wav_btn)

        # Combine and convert to M4B
        combine_m4b_btn = QPushButton("📚 Combine & Convert to M4B")
        combine_m4b_btn.setStyleSheet("QPushButton { background-color: #4CAF50; color: white; font-weight: bold; padding: 10px; }")
        combine_m4b_btn.clicked.connect(self.combine_audio)  # Keep existing functionality
        combine_buttons_layout.addWidget(combine_m4b_btn)

        layout.addLayout(combine_buttons_layout)
    ...
    def create_repair_tool_tab(self):
        ...
        # Chunk number search
        search_layout.addWidget(QLabel("Or search by chunk number:"))
        self.repair_chunk_number_edit = QLineEdit()
        self.repair_chunk_number_edit.setPlaceholderText("Enter chunk number (e.g. 47, 123)...")
        self.repair_chunk_number_edit.returnPressed.connect(self.search_chunks_by_number)
        search_layout.addWidget(self.repair_chunk_number_edit)

        # Search buttons
        search_btn_layout = QHBoxLayout()
        search_text_btn = QPushButton("🔍 Search Text")
        search_text_btn.clicked.connect(self.search_chunks_for_repair)
        search_btn_layout.addWidget(search_text_btn)

        search_number_btn = QPushButton("#️⃣ Search by Number")
        search_number_btn.clicked.connect(self.search_chunks_by_number)
        search_btn_layout.addWidget(search_number_btn)
    ...
    def apply_preset(self):
        ...
        try:
            # Respect user's current VADER choice; do not re-enable if they turned it off
            if self.vader_checkbox.isChecked():
    ...
    def start_conversion(self):
        ...
        # Clear any previous token overrun data
        try:
            from modules.terminal_logger import clear_token_overruns
            clear_token_overruns()
        except Exception:
            pass

        # Release any cached model before starting conversion (prevent dual loading)
        try:
            from modules.tts_engine import _release_global_tts_model, clear_voice_cache
            import torch
            import gc
            ...
            self.log_output("🧹 Released cached model before conversion")
        except Exception as e:
            self.log_output(f"⚠️ Warning during pre-conversion cleanup: {e}")
    ...
    def combine_audio_wav_only(self):
        """Combine audio chunks to WAV only (no M4B conversion)"""
        if not self.combine_book_edit.text():
            QMessageBox.warning(self, "Error", "Please select a book folder")
            return

        book_path = self.combine_book_edit.text()
        self.log_output(f"Combining audio to WAV only for: {Path(book_path).name}")
        self.update_status_display(f"🔗 Combining audio chunks to WAV for {Path(book_path).name}")

        try:
            # Import the WAV-only combine function
            from tools.combine_only import combine_audio_to_wav_only

            # Start combine in background with the selected book path
            self.process_thread = ProcessThread(combine_audio_to_wav_only, book_path)
            self.process_thread.output_signal.connect(self.log_output)
            self.process_thread.finished_signal.connect(lambda s, m: self.on_combine_wav_finished(s, m))
            self.process_thread.status_signal.connect(self.update_status_display)
            self.process_thread.start()
        except Exception as e:
            self.log_output(f"Error starting WAV combine: {e}")
            self.update_status_display(f"❌ Error starting WAV combine: {e}")
    ...
    def on_combine_wav_finished(self, success, message):
        """Handle WAV-only combine completion"""
        if success:
            self.log_output("✅ WAV combining completed successfully!")
            self.update_status_display("✅ WAV combining completed successfully!")
            QMessageBox.information(self, "Success", "Audio chunks combined to WAV successfully!")
        else:
            self.log_output(f"❌ WAV combine failed: {message}")
            self.update_status_display(f"❌ WAV combine failed: {message}")
            QMessageBox.critical(self, "Error", f"WAV combine failed:\n{message}")
    ...
    def load_chunks_for_repair(self):
        ...
        try:
            ...
            self.repair_chunk_number_edit.clear()
            self.current_repair_chunk = None
            self.update_repair_chunk_display()
    ...
    def search_chunks_by_number(self):
        """Search for chunks by chunk number"""
        if not self.current_repair_chunks:
            QMessageBox.warning(self, "No Chunks", "Please select a book first")
            return

        query = self.repair_chunk_number_edit.text().strip()
        if not query:
            QMessageBox.warning(self, "No Query", "Please enter chunk number(s) to search for")
            return

        try:
            # Parse comma-separated chunk numbers
            chunk_numbers = []
            for num_str in query.split(','):
                num_str = num_str.strip()
                if num_str.isdigit():
                    chunk_numbers.append(int(num_str) - 1)  # Convert to 0-based index

            if not chunk_numbers:
                QMessageBox.warning(self, "Invalid Input", "Please enter valid chunk numbers (e.g. 47, 123)")
                return

            # Find chunks by number
            results = []
            for chunk in self.current_repair_chunks:
                chunk_index = chunk.get('index', 0)
                if chunk_index in chunk_numbers:
            ...
            if results:
                for chunk in results:
                    ...
                    item.setData(Qt.UserRole, chunk)
                    self.repair_results_list.addItem(item)
                self.log_output(f"Found {len(results)} chunks by number")
            else:
                self.log_output("No chunks found with those numbers")

        except Exception as e:
            self.log_output(f"Error searching chunks by number: {e}")
...
Audio: chunk_{chunk['index']+1:05d}.wav"""
    ...
    def _handle_token_overruns(self, overruns):
        """Handle token overrun detection - show popup and optionally switch to repair tab"""
        from PyQt5.QtWidgets import QMessageBox, QPushButton

        overrun_count = len(overruns)
        chunk_list = ", ".join([f"#{chunk_num}" for chunk_num, _, _ in overruns])

        # Create detailed message
        title = "⚠️ Token Limit Exceeded - Audio May Be Truncated"
        message = (
            f"Some chunks used more tokens than the limit allows, which may result in "
            f"cut-off speech at the end of those segments.\n\n"
            f"Affected chunks ({overrun_count}): {chunk_list}\n\n"
            f"The affected audio will need to be regenerated to ensure complete speech.\n\n"
            f"Would you like to go to the Repair Tab to fix these chunks?"
        )
        ...
        # Add custom buttons
        repair_btn = QPushButton("Go to Repair Tab")
        cancel_btn = QPushButton("Cancel")

        msg_box.addButton(repair_btn, QMessageBox.AcceptRole)
        msg_box.addButton(cancel_btn, QMessageBox.RejectRole)

        # Show the dialog
        result = msg_box.exec_()

        if msg_box.clickedButton() == repair_btn:
            # Switch to repair tab (Tab 3)
            self.tabs.setCurrentIndex(2)  # 0=Main, 1=Config, 2=Repair
            self.log_output("🔧 Switched to Repair Tab - fix the overrun chunks and regenerate")
    ...
    def on_conversion_finished(self, success, message):
        ...
        if success:
            # Check for token overruns before proceeding
            try:
                from modules.terminal_logger import get_token_overruns
                overruns = get_token_overruns()

                if overruns:
                    # Log overruns to GUI output window
                    self.log_output("⚠️ TOKEN LIMIT EXCEEDED - Audio may be truncated:")
                    for chunk_num, tokens_used, token_limit in overruns:
                        self.log_output(f"   🚨 Chunk {chunk_num}: {tokens_used} tokens (limit: {token_limit})")

                    # Show popup and halt M4B conversion
                    self._handle_token_overruns(overruns)
                    return  # Stop here - no M4B conversion

            except Exception as e:
                self.log_output(f"⚠️ Error checking token overruns: {e}")
        ...
        # Release TTS model and clear VRAM after conversion completes
        try:
            from modules.tts_engine import _release_global_tts_model, clear_voice_cache
            import torch
            import gc
            ...
            self.log_output("🧹 Model released and VRAM cleared")
        except Exception as e:
            self.log_output(f"⚠️ Warning during cleanup: {e}")
    ...
    def reset_config_defaults(self):
        ...
        try:
            ...
            self.batch_chunk_size_spin.setValue(getattr(config_mod, 'BATCH_CHUNK_SIZE', self.batch_chunk_size_spin.value()))
            ...
            self.inline_toggle.setChecked(getattr(config_mod, 'ENABLE_INLINE_PAUSES', self.inline_toggle.isChecked()))
            self.inline_1_spin.setValue(getattr(config_mod, 'INLINE_PAUSE_1_MS', self.inline_1_spin.value()))
            self.inline_2_spin.setValue(getattr(config_mod, 'INLINE_PAUSE_2_MS', self.inline_2_spin.value()))
            ...
            self.vader_cfg_sens_spin.setValue(getattr(config_mod, 'VADER_CFG_WEIGHT_SENSITIVITY', self.vader_cfg_sens_spin.value()))
            self.vader_temp_sens_spin.setValue(getattr(config_mod, 'VADER_TEMPERATURE_SENSITIVITY', self.vader_temp_sens_spin.value()))

            # Parameter limits
            self.exag_min_spin.setValue(getattr(config_mod, 'TTS_PARAM_MIN_EXAGGERATION', self.exag_min_spin.value()))
            self.exag_max_spin.setValue(getattr(config_mod, 'TTS_PARAM_MAX_EXAGGERATION', self.exag_max_spin.value()))
    ...
    def save_original_config_values(self):
        ...
        self.original_config_values = {
            ...
            'batch_chunk_size': self.batch_chunk_size_spin.value(),
    ...
    def setup_config_change_tracking(self):
        ...
        config_widgets = [
            self.workers_spin, self.batch_size_spin, self.tts_batch_size_spin, self.batch_chunk_size_spin,
            self.max_new_tokens_spin, self.min_chunk_words_spin, self.max_chunk_words_spin, self.target_lufs_spin, self.speech_threshold_spin,
    ...
    def save_config_to_file(self):
        ...
        try:
            ...
            gui_values = {
                ...
                # Inline pause settings
                'ENABLE_INLINE_PAUSES': self.inline_toggle.isChecked(),
                'INLINE_PAUSE_1_MS': self.inline_1_spin.value(),
                'INLINE_PAUSE_2_MS': self.inline_2_spin.value(),
            }
    ...
    def generate_from_json(self):
        ...
        try:
            ...
            # Analyze JSON for optimal token settings
            self.log_output("📊 Analyzing chunks for optimal token settings...")
            try:
                from modules.token_analyzer import analyze_and_optimize_tokens, format_analysis_summary
                from pathlib import Path

                analysis_results = analyze_and_optimize_tokens(Path(json_path), auto_update=False)
                summary = format_analysis_summary(analysis_results)
                self.log_output(summary)

                if analysis_results['success']:
                    optimal_tokens = analysis_results['optimal_max_tokens']
                    current_tokens = self.max_new_tokens_spin.value()
                    self.log_output(f"🎯 Analysis suggests {optimal_tokens} tokens (current: {current_tokens})")
                    self.log_output(f"🔧 Using Tab 2 setting: {current_tokens} tokens")
                else:
                    self.log_output("⚠️ Token analysis failed - using Tab 2 setting")

            except Exception as e:
                self.log_output(f"⚠️ Token analysis failed: {e} - using current settings")
